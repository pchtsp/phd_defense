\begin{tikzpicture}
       \only<+->{
            ...
            \node [anchor=east] (fp) at ([yshift=1cm]fpc.west) {Flight potential};
            \draw [bend left=50, ->] (fpc) to (fp);

            \node [anchor=west, fill=none, scale=2] (cpc) at ([xshift=2pt]exa) {};
            \node [anchor=east] (cp) at ([yshift=-1.5cm]cpc.east) {Calendar potential};
            \draw [bend left=50, ->] (cpc.east) to (cp.east);

            \node [] at ([xshift=0.5cm]exa.east) {$\star$};
        }
       % \onslide<1->{
       %  }
       \onslide<+->{
