...
\frametitle{\textbf{Predicting pseudo-cuts}}
  ...
  \begin{columns}[c]
    ...
    \textbf{Given an optimization problem :}
  ...
  \onslide<3->{
  % \begin{block}{We obtain solution features}
    % \begin{equation*}
  ...
  \begin{columns}[c]
    \column{0.6\linewidth}
    \textbf{We obtain solution features :}
    \column{0.4\linewidth}
    \begin{equation*}
      g_n(y) \,\, \forall n \in \{1, .., N\}
    \end{equation*}
  ...
  \onslide<4->{
  % \begin{block}{We predict them from input data}
  %   \begin{equation*}
  ...
  \onslide<5->{
  % \begin{block}{We use those features to produce and solve a smaller problem}
  %   \begin{align*}
  %     & \mathcal{Y}^\prime(x) = \{y \in \mathcal{Y} \mid \hat{g}_n(x) = g_n(y)\} \\
  ...
  \onslide<6->{
  % \begin{block}{While keeping an acceptable loss of optimality}
  %   \begin{equation*}
